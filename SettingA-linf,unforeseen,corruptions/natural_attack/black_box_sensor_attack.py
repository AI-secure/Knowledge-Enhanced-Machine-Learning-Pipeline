...
ckpt = torch.load('../substitute/sub.ckpt', map_location = 'cuda') # black-box sensor attack
...
for attack_type in attack_mapping.keys():
    ...
    while st!= num:
        ...
        random_target_batch = np.random.randint(0,n_class,[this_batch_size])#np.full([this_batch_size],11,dtype=np.long)
        for i in range(this_batch_size):
            while random_target_batch[i]==gt_batch[i]:
                random_target_batch[i] = np.random.randint(0,n_class)
        random_target_batch = torch.LongTensor(random_target_batch).cuda()
