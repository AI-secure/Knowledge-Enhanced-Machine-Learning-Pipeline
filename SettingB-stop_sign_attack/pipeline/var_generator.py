...
mapping = {
    ...
    17: 6, # no entry
    15: 7, # no vehicles
    2: 8, # speed limit 50km/h
    32: 9, # end of all speed and passing limits
    0: 10, # speed limit 20km/h
    8: 11, # speed limit 120km/h
}
    ...
    def gen_vars(self,signal_list):
        ...
        while(st!=num):
            ...
            for i in range(st,ed):
                ori_label = int(Y[i-st])
                if ori_label in mapping.keys():
                    Vars[i][0] = mapping[ori_label]
                else:
                    Vars[i][0] = -1
                if Vars[i][0] == GT[i]: correct+=1
            st = ed
            ed = min(st+batch_size,num)
